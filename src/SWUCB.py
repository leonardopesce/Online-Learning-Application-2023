...
class SWUCBLearner(UCBLearner):
    ...
    def update(self, pulled_arm, reward, bernoulli_realization):
        ...
        for arm in range(self.n_arms):
            ...
            self.empirical_means[arm] = np.sum(self.rewards_per_arm[arm][-n_samples:]) / n_samples if n_samples > 0 else 0
            self.confidence[arm] = 100 * np.sqrt((2 * np.log(self.t) / n_samples)) if n_samples > 0 else np.inf
    ...
    def get_conv_prob(self, pulled_arm):
        ...
        return np.sum(self.successes_per_arm[pulled_arm][-n_samples:]) / np.sum(self.total_observations_per_arm[pulled_arm][-n_samples:]) if np.sum(self.total_observations_per_arm[pulled_arm][-n_samples:]) > 0 else 1
