...
for e in tqdm(range(0, n_experiments)):
    ...
    for t in range(0, T):
        ...
        env_class = classes[np.random.randint(0, 3)]
        # TS Learner
        price_idx, bid_idx = ts_learner[env_class].pull_arm(env.other_costs)
        # this may simulate that all the users are of the same class
        bernoulli_realizations, n_clicks, cum_daily_cost = env.round(env_class, price_idx, bid_idx)
        reward = env.get_reward(env_class, price_idx, np.mean(bernoulli_realizations), n_clicks, cum_daily_cost)
        ts_learner[env_class].update(price_idx, bernoulli_realizations, bid_idx, n_clicks, cum_daily_cost, reward)

        # UCB Learner
        price_idx, bid_idx = ucb_learner[env_class].pull_arm(env.other_costs)
        bernoulli_realizations, n_clicks, cum_daily_cost = env.round(env_class, price_idx, bid_idx)
        reward = env.get_reward(env_class, price_idx, np.mean(bernoulli_realizations), n_clicks, cum_daily_cost)
        ucb_learner[env_class].update(price_idx, bernoulli_realizations, bid_idx, n_clicks, cum_daily_cost, reward)
